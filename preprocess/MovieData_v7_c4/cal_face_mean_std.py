...
def get_trn_val(target_root_dir, setname):
    int2name = np.load(osp.join(target_root_dir, '1', '{}_int2name.npy'.format(setname)))
    int2label = np.load(osp.join(target_root_dir, '1', '{}_label.npy'.format(setname)))
...
def calc_mean_std(config):
    ...
    trn_int2name, _ = get_trn_val(target_root, 'trn')
    #val_int2name, _ = get_trn_val(target_root, 'val')
