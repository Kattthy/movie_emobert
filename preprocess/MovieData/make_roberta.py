...
class RobertaExtractor(object):
    def __init__(self, cuda=False, cuda_num=None):
        self.tokenizer = RobertaTokenizer.from_pretrained('/data8/hzp/RoBERTa/output/roberta_3/')
        self.model = RobertaModel.from_pretrained('/data8/hzp/RoBERTa/output/roberta_3/')
...
def make_all_roberta(config):
    extractor = RobertaExtractor(cuda=True, cuda_num=0)
    ...
    mkdir(os.path.join(config['feature_root'], 'L', 'roberta'))
    all_h5f = h5py.File(os.path.join(config['feature_root'], 'L', 'roberta', 'all.h5'), 'w')
...
if __name__ == '__main__':
    ...
    # pwd = os.path.dirname(pwd) #获取该文件的上级目录
    # config_path = os.path.join(pwd, '../', 'data/config', 'MovieData_config.json')
    # config = json.load(open(config_path))
    # make_all_roberta(config)
    a = RobertaExtractor()
