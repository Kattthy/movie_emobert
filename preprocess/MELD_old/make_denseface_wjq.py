...
class DensefaceExtractor(object):
    ...
    def __init__(self, restore_path=None, mean=52.208, std=43.833, device=0):
    ...
    def __call__(self, config):
        trn_int2name, _ = get_trn_val_tst(config['target_root'], 'trn')
        val_int2name, _ = get_trn_val_tst(config['target_root'], 'val')
        tst_int2name, _ = get_trn_val_tst(config['target_root'], 'tst')
        ...
        trn_h5f = h5py.File(os.path.join(config['feature_root'], 'V', 'denseface', 'trn.h5'), 'w')
        tst_h5f = h5py.File(os.path.join(config['feature_root'], 'V', 'denseface', 'tst.h5'), 'w')
        val_h5f = h5py.File(os.path.join(config['feature_root'], 'V', 'denseface', 'val.h5'), 'w')
        ...
        for utt_id in tqdm(tst_int2name):
            face_dir = os.path.join(config['data_root'], 'faces', 'test', utt_id, utt_id+'_aligned')
            face_lst = self.get_faces(face_dir)
            if len(face_lst) == 0:
                tst_h5f[utt_id] = np.zeros((1, 342))
                continue
            face_tensor = self.get_face_tensors(face_lst) #face_tensor = torch.from_numpy(self.get_face_tensors(face_lst)).to(device)
            #这一步当中已经对读入的face进行了归一化
        ...
        for utt_id in tqdm(val_int2name):
            face_dir = os.path.join(config['data_root'], 'faces', 'dev', utt_id, utt_id+'_aligned')
            face_lst = self.get_faces(face_dir)
            if len(face_lst) == 0:
                val_h5f[utt_id] = np.zeros((1, 342))
                continue
            face_tensor = self.get_face_tensors(face_lst) #face_tensor = torch.from_numpy(self.get_face_tensors(face_lst)).to(device)
            #这一步当中已经对读入的face进行了归一化
