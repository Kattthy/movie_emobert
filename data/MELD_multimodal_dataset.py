...
class MELDMultimodalDataset(BaseDataset):
    @staticmethod
    ...
    def __init__(self, opt, set_name):
        ...
        config = json.load(open('/data8/hzp/movie_emobert/data/config/MELD_config.json'))
        self.norm_method = opt.norm_method
        self.A_type = opt.A_type
        self.all_A = h5py.File(os.path.join(config['feature_root'], 'A', f'{self.A_type}', self.set_name+'.h5'), 'r')
        ...
        self.all_V = h5py.File(os.path.join(config['feature_root'], 'V', f'{self.V_type}', self.set_name+'.h5'), 'r')
        self.L_type = opt.L_type
        self.all_L = h5py.File(os.path.join(config['feature_root'], 'L', f'{self.L_type}', self.set_name+'.h5'), 'r')
        ...
        print(f"MELD dataset {set_name} created with total length: {len(self)}")
...
if __name__ == '__main__':
    ...
    a = MELDMultimodalDataset(opt, set_name='val')
    data = next(iter(a))
