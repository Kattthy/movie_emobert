...
def create_dataset_with_args(opt, **kwargs):
    ...
    for key in kwargs:
        ...
        for i, v in enumerate(value):
            _kwargs[i][key] = v 
            #_kwargs[0]['set_name']='trn'
            #_kwargs[1]['set_name']='val'
            #_kwargs[2]['set_name']='tst'
        
    dataloaders = tuple(map(lambda x: CustomDatasetDataLoader(opt, **x), _kwargs)) #分别对每个集合进行dataloader的操作
    return dataloaders if len(dataloaders) > 1 else dataloaders[0]


class CustomDatasetDataLoader(): #应该在这里对kwargs传入的数据集进行判断，如果是'trn'则进行相应的修改
