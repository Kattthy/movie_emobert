...
for i in `seq 1 1 1`;
do
cmd="python finetune_baseline_nocv.py --dataset_mode=MELD_multimodal --model=utt_fusion
...
--output_dim=5 --cls_layers=128,128 --dropout_rate=0.3
--niter=20 --niter_decay=20 --verbose --beta1=0.9 --init_type normal
--batch_size=256 --lr=5e-4 --run_idx=$run_idx
--name=finetune_MELD_baseline_cnn --suffix={modality}_run{run_idx} --has_test
--cvNo=$i"
...
#运行命令：bash scripts/utt_fusion_finetune_MELD.sh AVL 1 0。表示跑AVL三模态融合的实验, 实验编号是1, 在0号卡上跑