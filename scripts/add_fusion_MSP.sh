...
for i in `seq 1 1 12`;
do
cmd="python train_baseline_cv12.py --dataset_mode=MSP_multimodal --model=add_fusion
...
--name=MSP_add_5e-4 --suffix={modality}_run{run_idx} --has_test
--cvNo=$i"
...
#运行命令：bash scripts/add_fusion_MSP.sh AVL 1 0。表示跑AVL三模态融合的实验, 实验编号是1, 在0号卡上跑