...
cmd="python train_baseline_nocv.py --dataset_mode=MovieData_v7_expt_multimodal --model=utt_fusion
...
--name=MovieData_v7_expt3_concat_5e-4 --suffix={modality}_run{run_idx}
--cvNo=$i"
...
#运行命令：bash scripts/utt_fusion_MovieData_v7_expt.sh AVL 1 1。表示跑AVL三模态融合的实验, 实验编号是1, 在1号卡上跑