...
--name=finetune_MSP_expt1_concat_5e-4 --suffix={modality}_run{run_idx} --has_test
--cvNo=$i"
...
#运行命令：bash scripts/utt_fusion_finetune_MSP_expt.sh AVL 1 1。表示跑AVL三模态融合的实验, 实验编号是1, 在0号卡上跑