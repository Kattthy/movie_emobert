...
cmd="python train_baseline_nocv.py --dataset_mode=MovieData_multimodal --model=utt_fusion
...
--name=MovieData_baseline_cnn --suffix={modality}_run{run_idx} --has_test
--cvNo=$i"
...
#运行命令：bash scripts/utt_fusion_MovieData_v7.sh AVL 1 0。表示跑AVL三模态融合的实验, 实验编号是1, 在0号卡上跑