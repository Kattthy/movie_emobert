...
class BaseModel(ABC):
    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            ...
            for name in self.model_names:
                ...
                tools.init_weights(net, opt.init_type) #hzp add
    ...
    def finetune(self, opt, experiment_name, epoch):
        '''
        hzp add
        既做setup的事儿，又做load_networks的事儿。

        experiment_name为要用作预训练模型的那组训练实验的checkpoint以及log中目录的名称
        epoch (int) -- 预训练模型验证集效果最好的那个轮次
        '''
        if self.isTrain:
            self.schedulers = [tools.get_scheduler(optimizer, opt) for optimizer in self.optimizers]
            ...
            for name in self.model_names:
                if isinstance(name, str):
                    load_filename = '%s_net_%s.pth' % (epoch, name)
                    load_path = os.path.join(opt.checkpoints_dir, experiment_name, load_filename)
                    net = getattr(self, 'net' + name)
                    ...
                    if isinstance(net, torch.nn.DataParallel): #目前就是这两句导致net不再是parallel了，但是注释掉的话net.load_state_dict(state_dict)那句又会出问题
                        net = net.module
                    ...
                    print('loading the model from %s' % load_path)
                    #state_dict = torch.load(load_path, map_location=self.device)
                    state_dict = torch.load(load_path)#hzp modified
                    ...
                    net = tools.init_net(net, opt.gpu_ids) #hzp add
    ...
    def set_requires_grad(self, nets, requires_grad=False): #相当于with.no_grad
