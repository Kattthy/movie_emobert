...
from models.networks.attn import ModalityFusionAttn
...
class AttentionFusionModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netattn = ModalityFusionAttn(128)
        # acoustic model
    ...
    def forward(self):
        ...
        # get model outputs
        self.attn_fusion = self.netattn(*final_embd) #实参加*，是把原本的列表打散
        #self.feat = torch.cat(final_embd, dim=-1)
        #self.logits, self.ef_fusion_feat = self.netC(self.feat)
        self.logits, self.ef_fusion_feat = self.netC(self.attn_fusion)
