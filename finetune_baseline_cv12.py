...
if __name__ == '__main__':
    ...
    #这里需要每次手动修改
    model.finetune(opt, os.path.join('MovieData_v7_expt1_concat_5e-4_AVL_run1', '1'), 9) #opt, (checkpoint_name, cvNo), epoch(int)
    ...
    if opt.has_test:
        ...
        _ = eval(model, val_dataset, is_save=True, phase='val') #如果无测试过程，也要保留最好的验证集
        #如果没有测试过程，文件中记录的就是最好的验证集的结果
        result_recorder.write_result_to_tsv({
            'acc': best_epoch_acc,
            'uar': best_eval_uar,
            'f1': best_epoch_f1
